...
create_PDF<-function(Labels = NA, ErrCorr="H",Across=T,Fsz=2.5,trunc=T,ERows=0,ECols=0){
  ...
  if(length(Labels) == 0){
    ...
    ask <- noquote(toupper(readline("Do you want to use custom values to create pdf? (T/F): ")))
    
    while((ask %in% inputCheck)==FALSE){
      noquote(print("Invalid input"))
      ask <- noquote(toupper(readline("Do you want to custom parameters for pdf? (T/F): ")))
    }
