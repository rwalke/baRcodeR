---
title: "baRcodeR - Open-source label generation for tracking biological samples"
author: "Yihan Wu"
date: "`r Sys.Date()`"
...
The baRcodeR package will generate unique IDs with 2-dimensional barcode tags in a pdf output format that can be printed using a laser printer and sticker sheets. Each function can be run interactively with prompts to guide unique ID code and barcode label creation. 
...
Creating unique, scannable 2-D barcodes involves two steps:
  ...
  2. Create a pdf file containing unique ID codes coupled with 2D barcode with `create_PDF()`

If you already have ID codes in a text file, skip step #1 and import them into a `data.frame()` object for step #2: see [Create barcodes](#create-barcodes).
...
Simple one-level ID codes can be created using the 'uniqID_maker()' function. One-level ID codes consist of two parts, a leading string shared by all samples and a number unique to each sample. 
...
The string entered here defines the leading part of the ID code that will be the same for every label produced. This can be the name of a population, a species or a location. In this example, the string entered by the user into the console as denoted by the `>` symbol is "example".
...
The prompt asks for the ending number. Unique IDs are generated sequentially from the starting number to the ending number. Note that a higher number can be used as the starting number to generate a reverse order. It is possible to generate IDs that are not sequentially numbered by passing a vector through `uniqID_maker()` (e.g. `seq()`) (see [By arguments](#label-maker-arguments)) 
...
It is also possible to create unique ID codes without user prompts by defining parameters directly in `uniqID_maker`. The interactive example above can be reproduced by the code:

```{r eval=T, include=T}
Labels <- uniqID_maker(string = "example", level = 5:10, digits = 3)
Labels
```
...
Labels <- uniqID_maker(string = "example", level = number_sequence, digits = 3)
```
...
```{r eval=F, include=T}
Labels <- uniqID_hier_maker(user = T)
```
...
The list can then be passed into `uniqID_hier_maker`.

```{r, eval=T, include=T}
Labels <- uniqID_hier_maker(hierarchy = hier_list, digits = 1)
```
...
2D barcodes are created from a set of text labels. Users can manually create their own labels in a the first column of a `data.frame()` object or create labels using `uniqID_maker` or `uniqID_hier_maker`. 
...
The first step is to read in the labels, whether from a csv file or the output of `uniqID_maker` or `uniqID_hier_maker`. 
