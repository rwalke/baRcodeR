...
custom_create_PDF<-function(user=F, Labels = NA, name="LabelsOut", ErrCorr="H", Fsz=2.5,Across=T,ERows=0,ECols=0,trunc=T, numrow=20, numcol=4, height_margin=0.5, width_margin=0.25, cust_spacing=F, x_space=215){
  ...
  grid::pushViewport(viewport(width=unit(width_margin,"in"),height=unit(height_margin,"in"),just=c("centre","centre"),layout = grid.layout(numrow, numcol))) # Margins: left/right:10mm x top/bottom:22mm
  row<-ERows
  col<-ECols+1
