...
test_that("input errors are produced", {
  ...
  expect_error(label_maker(string="example", level = c(1:5), digits="a"), "Digits is not a numerical value")
  expect_warning(label_maker(string="example", level=1:300, digits=2), "Digits specified less than max number. Increasing number of digits.")
  ...
  expect_error(label_hier_maker(hierarchy = list(c("a",3,6), c("b",1,"j"))), "Invalid ending number on level")
